$(function () {
  ...
  QUnit.test('should activate element by tab id in nav list', function (
    assert
  ) {
    assert.expect(2);
    var tabsHTML =
    ...
    $('<nav><div id="home"></div><div id="profile"></div></nav>').appendTo(
      '#qunit-fixture'
    );
  ...
  QUnit.test('should activate element by tab id in list group', function (
    assert
  ) {
    assert.expect(2);
    var tabsHTML =
  ...
  QUnit.test('should not fire shown when tab is already active', function (
    assert
  ) {
    assert.expect(0);
    var tabsHTML =
  ...
  QUnit.test('selected tab should have aria-selected', function (assert) {
    ...
    assert.strictEqual(
      $tabs.find('.active').attr('aria-selected'),
      'true',
      'shown tab has aria-selected = true'
    );
    assert.strictEqual(
      $tabs.find('a:not(.active)').attr('aria-selected'),
      'false',
    ...
    assert.strictEqual(
      $tabs.find('.active').attr('aria-selected'),
      'true',
      'after click, shown tab has aria-selected = true'
    );
    assert.strictEqual(
      $tabs.find('a:not(.active)').attr('aria-selected'),
      'false',
    ...
    assert.strictEqual(
      $tabs.find('.active').attr('aria-selected'),
      'true',
      'after second show event, shown tab still has aria-selected = true'
    );
    assert.strictEqual(
      $tabs.find('a:not(.active)').attr('aria-selected'),
      'false',
  ...
  QUnit.test('selected tab should deactivate previous selected tab', function (
    assert
  ) {
    assert.expect(2);
    var tabsHTML =
  ...
  QUnit.test(
    'selected tab should deactivate previous selected link in dropdown',
    function (assert) {
      assert.expect(3);
      var tabsHTML =
        '<ul class="nav nav-tabs">' +
      ...
      assert.notOk(
        $tabs
          .find('li:last-child .dropdown-menu a:first-child')
          .hasClass('active')
      );
    }
  );
  ...
  QUnit.test(
    'should not add show class to tab panes if there is no `.fade` class',
    function (assert) {
      assert.expect(1);
      var done = assert.async();
