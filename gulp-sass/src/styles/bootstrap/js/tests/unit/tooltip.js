$(function () {
  ...
  QUnit.test(
    'should throw an error when show is called on hidden elements',
    function (assert) {
      assert.expect(1);
      var done = assert.async();
      ...
      try {
        $(
          '<div title="tooltip title" style="display: none"/>'
        ).bootstrapTooltip('show');
      } catch (err) {
  ...
  QUnit.test(
    'should place tooltips inside a specific container when container is an element',
    function (assert) {
      assert.expect(3);
      var done = assert.async();
  ...
  QUnit.test(
    'should place tooltips inside a specific container when container is a selector',
    function (assert) {
      assert.expect(3);
      var done = assert.async();
      var $container = $('<div id="container"></div>').appendTo(
        '#qunit-fixture'
      );
      var $tooltip = $('<a href="#" rel="tooltip" title="Another tooltip"/>')
        .appendTo('#qunit-fixture')
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      setTimeout(function () {
        assert.ok(
          !$('.tooltip').is('.fade.show'),
          '100ms: tooltip is not faded active'
        );
      }, 100);
    ...
    function (assert) {
      ...
      setTimeout(function () {
        ...
        setTimeout(function () {
          assert.ok(
            $('.tooltip').is('.fade.show'),
            '100ms: tooltip still faded active'
          );
          $tooltip.trigger('mouseenter');
        }, 100);
        ...
        setTimeout(function () {
          assert.ok(
            $('.tooltip').is('.fade.show'),
            '200ms: tooltip still faded active'
          );
          done();
        }, 200);
      }, 0);
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      setTimeout(function () {
        assert.ok(
          !$('.tooltip').is('.fade.show'),
          '250ms: tooltip not faded active'
        );
        done();
      }, 250);
  ...
  QUnit.test('should wait 200ms before hiding the tooltip', function (assert) {
    ...
    setTimeout(function () {
      assert.ok(
        $($tooltip.data('bs.tooltip').tip).is('.fade.show'),
        '1ms: tooltip faded active'
      );
      ...
      setTimeout(function () {
        assert.ok(
          !$($tooltip.data('bs.tooltip').tip).is('.show'),
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      $('#tt-outer').trigger('mouseenter');
      assert.ok(
        obj._hoverState === 'show',
        'the tooltip hoverState should be set to "show"'
      );
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      assert.strictEqual(
        typeof $tooltip.data('bs.tooltip'),
        'undefined',
  ...
  QUnit.test(
    'tooltip should be shown right away after the call of disable/enable',
    function (assert) {
      assert.expect(2);
      var done = assert.async();
  ...
  QUnit.test(
    'should use Popper.js to get the tip on placement change',
    function (assert) {
      assert.expect(1);
  ...
  QUnit.test(
    'should allow passing a custom function for sanitization',
    function (assert) {
      assert.expect(1);
  ...
  QUnit.test(
    'should not take into account sanitize in data attributes',
    function (assert) {
      assert.expect(1);

      var $trigger = $(
        '<a href="#" rel="tooltip" data-sanitize="false" data-trigger="click" title="Another tooltip"/>'
      )
        .appendTo('#qunit-fixture')
        .bootstrapTooltip({
          template: ['<span aria-pressed="true">Some content</span>'].join('')
