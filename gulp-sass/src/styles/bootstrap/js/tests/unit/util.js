$(function () {
  ...
  QUnit.test(
    'Util.getSelectorFromElement should return the correct element',
    function (assert) {
      assert.expect(2);

      var $el = $('<div data-target="body"></div>').appendTo(
        $('#qunit-fixture')
      );
      assert.strictEqual(Util.getSelectorFromElement($el[0]), 'body');

      // Not found element
      var $el2 = $('<div data-target="#fakeDiv"></div>').appendTo(
        $('#qunit-fixture')
      );
  ...
  QUnit.test(
    'Util.getSelectorFromElement should return null when there is a bad selector',
    function (assert) {
      assert.expect(2);
  ...
  QUnit.test(
    'Util.isElement should check if we passed an element or not',
    function (assert) {
      assert.expect(3);
  ...
  QUnit.test(
    'Util.getTransitionDurationFromElement should accept transition durations in milliseconds',
    function (assert) {
      assert.expect(1);
      var $div = $(
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      var $div = $(
        '<div style="transition: all 0s 150ms ease-out;"></div>'
      ).appendTo($fixture);
      var $div2 = $(
  ...
  QUnit.test('Util.supportsTransitionEnd should return true', function (
    assert
  ) {
    assert.expect(1);
  ...
  QUnit.test('Util.findShadowRoot should find the shadow DOM root', function (
    assert
  ) {
