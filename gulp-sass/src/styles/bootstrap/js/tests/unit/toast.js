$(function () {
  ...
  QUnit.test('should provide no conflict', function (assert) {
    ...
    assert.strictEqual(
      typeof $.fn.toast,
      'undefined',
      'toast was set back to undefined (org value)'
    );
  });
  ...
  QUnit.test(
    'should do nothing when we call hide on a non shown toast',
    function (assert) {
      assert.expect(1);
  ...
  QUnit.test(
    'should close toast when close element with data-dismiss attribute is set',
    function (assert) {
      assert.expect(2);
      var done = assert.async();
  ...
  QUnit.test(
    'should expose default setting to allow to override them',
    function (assert) {
      assert.expect(1);
