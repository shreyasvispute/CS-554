$(function () {
  ...
  QUnit.module('modal', {
    before: function () {
      ...
      $(
        '<style type="text/css"> .modal-scrollbar-measure { position: absolute; top: -9999px; width: 50px; height: 50px; overflow: scroll; } </style>'
      ).appendTo('head');
      // Function to calculate the scrollbar width which is then compared to the padding or margin changes
      $.fn.getScrollbarWidth =
        $.fn.modal.Constructor.prototype._getScrollbarWidth;
  ...
  QUnit.test(
    'should not close modal when clicking outside of modal-content if data-backdrop="true"',
    function (assert) {
      assert.expect(1);
      var done = assert.async();
  ...
  QUnit.test(
    'should add aria-modal attribute when shown, remove it again when hidden',
    function (assert) {
      assert.expect(3);
      var done = assert.async();
      ...
      $('<div id="modal-test"/>')
        .on('shown.bs.modal', function () {
          assert.ok(
            $('#modal-test').is('[aria-modal]'),
            'aria-modal attribute added'
          );
          assert.strictEqual(
            $('#modal-test').attr('aria-modal'),
            'true',
            'correct aria-modal="true" added'
          );
          $(this).bootstrapModal('hide');
        })
        .on('hidden.bs.modal', function () {
  ...
  QUnit.test(
    'should adjust the inline padding of the modal when opening',
    function (assert) {
      assert.expect(1);
      var done = assert.async();
      ...
      $('<div id="modal-test"/>')
        .on('shown.bs.modal', function () {
          ...
          assert.strictEqual(
            currentPadding,
            expectedPadding,
            'modal padding should be adjusted while opening'
          );
          done();
        })
        .bootstrapModal('show');
      ...
      $('<div id="modal-test"/>')
        .on('hidden.bs.modal', function () {
          ...
          assert.strictEqual(
            currentPadding,
            originalPadding,
            'body padding should be reset after closing'
          );
        ...
        .on('shown.bs.modal', function () {
          var expectedPadding =
            parseFloat(originalPadding) + $(this).getScrollbarWidth() + 'px';
          var currentPadding = $body.css('padding-right');
          assert.strictEqual(
            currentPadding,
            expectedPadding,
            'body padding should be adjusted while opening'
          );
          $(this).bootstrapModal('hide');
        })
        .bootstrapModal('show');
  ...
  QUnit.test(
    'should store the original body padding in data-padding-right before showing',
    function (assert) {
      assert.expect(2);
      var done = assert.async();
      var $body = $(document.body);
      ...
      $('<div id="modal-test"/>')
        .on('hidden.bs.modal', function () {
          assert.strictEqual(
            typeof $body.data('padding-right'),
            'undefined',
        ...
        .on('shown.bs.modal', function () {
          assert.strictEqual(
            ...
            originalPadding,
            'original body padding should be stored in data-padding-right'
          );
          $(this).bootstrapModal('hide');
        })
        .bootstrapModal('show');
  ...
  QUnit.test(
    ...
    function (assert) {
      assert.expect(2);
      var done = assert.async();
      var $element = $('<div class="fixed-top"></div>').appendTo(
        '#qunit-fixture'
      );
  ...
  QUnit.test(
    'should store the original padding of fixed elements in data-padding-right before showing',
    function (assert) {
      assert.expect(2);
      var done = assert.async();
      var $element = $('<div class="fixed-top"></div>').appendTo(
      ...
      $('<div id="modal-test"/>')
        .on('hidden.bs.modal', function () {
          assert.strictEqual(
            typeof $element.data('padding-right'),
            'undefined',
            'data-padding-right should be cleared after closing'
          );
          $element.remove();
        ...
        .on('shown.bs.modal', function () {
          assert.strictEqual(
            ...
            originalPadding,
            'original fixed element padding should be stored in data-padding-right'
          );
          $(this).bootstrapModal('hide');
        })
        .bootstrapModal('show');
  ...
  QUnit.test(
    ...
    function (assert) {
      assert.expect(2);
      var done = assert.async();
      var $element = $('<div class="sticky-top"></div>').appendTo(
        '#qunit-fixture'
      );
  ...
  QUnit.test(
    'should store the original margin of sticky elements in data-margin-right before showing',
    function (assert) {
      assert.expect(2);
      var done = assert.async();
      var $element = $('<div class="sticky-top"></div>').appendTo(
      ...
      $('<div id="modal-test"/>')
        .on('hidden.bs.modal', function () {
          assert.strictEqual(
            typeof $element.data('margin-right'),
            'undefined',
        ...
        .on('shown.bs.modal', function () {
          assert.strictEqual(
            ...
            originalPadding,
            'original sticky element margin should be stored in data-margin-right'
          );
          $(this).bootstrapModal('hide');
        })
        .bootstrapModal('show');
  ...
  QUnit.test(
    ...
    function (assert) {
      ...
      $('<div id="modal-test"/>')
        .on('hidden.bs.modal', function () {
          assert.strictEqual(
            $body.attr('style').indexOf('padding-right'),
            -1,
  ...
  QUnit.test('should not parse target as html', function (assert) {
    ...
    var $toggleBtn = $(
      '<button data-toggle="modal" data-target="&lt;div id=&quot;modal-test&quot;&gt;&lt;div class=&quot;contents&quot;&lt;div&lt;div id=&quot;close&quot; data-dismiss=&quot;modal&quot;/&gt;&lt;/div&gt;&lt;/div&gt;"/>'
    ).appendTo('#qunit-fixture');
    ...
    setTimeout(function () {
      assert.strictEqual(
        $('#modal-test').length,
        0,
        'target has not been parsed and added to the document'
      );
      done();
    }, 0);
  });
  ...
  QUnit.test('should not execute js from target', function (assert) {
    ...
    var $toggleBtn = $(
      '<button data-toggle="modal" data-target="&lt;div&gt;&lt;image src=&quot;missing.png&quot; onerror=&quot;$(&apos;#qunit-fixture button.control&apos;).trigger(&apos;click&apos;)&quot;&gt;&lt;/div&gt;"/>'
    ).appendTo('#qunit-fixture');
  ...
  QUnit.test(
    'should not try to open a modal which is already visible',
    function (assert) {
      assert.expect(1);
      var done = assert.async();
      var count = 0;
  ...
  QUnit.test('should dispose modal', function (assert) {
    ...
    $modal
      .on('shown.bs.modal', function () {
        ...
        assert.ok(
          typeof $(this).data('bs.modal') === 'undefined',
          'modal data object was disposed'
        );

        assert.ok(spy.callCount === 4, '`jQuery.off` was called');

        assert.ok(
          modalDataApiEvent.length === 1,
          '`Event.CLICK_DATA_API` on `document` was not removed'
        );
  ...
  QUnit.test(
    'should scroll to top of the modal body if the modal has .modal-dialog-scrollable class',
    function (assert) {
      assert.expect(2);
      var done = assert.async();
      ...
      $modal
        .on('shown.bs.modal', function () {
          assert.strictEqual(
            $modalBody.scrollTop(),
            0,
            'modal body scrollTop should be 0 when opened'
          );
          done();
        })
        .bootstrapModal('show');
