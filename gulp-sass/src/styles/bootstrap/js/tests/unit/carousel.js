$(function () {
  ...
  window.Carousel =
    typeof bootstrap !== 'undefined' ? bootstrap.Carousel : Carousel;

  var originWinPointerEvent = window.PointerEvent;
  window.MSPointerEvent = null;
  var supportPointerEvent = Boolean(
  ...
  QUnit.test('should reset when slide is prevented', function (assert) {
    ...
    $carousel
      .one('slide.bs.carousel', function (e) {
        ...
        setTimeout(function () {
          assert.ok(
            $carousel.find('.carousel-item:nth-child(1)').is('.active'),
            'first item still active'
          );
          assert.ok(
            $carousel
              .find('.carousel-indicators li:nth-child(1)')
              .is('.active'),
      ...
      .one('slid.bs.carousel', function () {
        setTimeout(function () {
          ...
          assert.ok(
            $carousel.find('.carousel-item:nth-child(2)').is('.active'),
            'second item active'
          );
          assert.ok(
            $carousel
              .find('.carousel-indicators li:nth-child(2)')
              .is('.active'),
  ...
  QUnit.test('should fire slid and slide events with from and to', function (
    assert
  ) {
    assert.expect(4);
    var template =
  ...
  QUnit.test(
    'should set interval from data attribute on individual carousel-item',
    function (assert) {
      assert.expect(2);
      var templateHTML =
        '<div id="myCarousel" class="carousel slide" data-interval="1814">' +
        '<div class="carousel-inner">' +
      ...
      assert.strictEqual(
        $carousel.data('bs.carousel')._config.interval,
        1814,
        'reverts to default interval if no data-interval is set'
      );
      $carousel.remove();
    }
  );
  ...
  QUnit.test('should not prevent keydown for inputs and textareas', function (
    assert
  ) {
    assert.expect(2);
    var templateHTML =
  ...
  QUnit.test(
    'should not go to the next item when the carousel is not visible',
    function (assert) {
      assert.expect(2);
      var done = assert.async();
      var html =
  ...
  QUnit.test(
    'should not go to the next item when the parent of the carousel is not visible',
    function (assert) {
      assert.expect(2);
      var done = assert.async();
      var html =
  ...
  QUnit.test(
    'should allow swiperight and call prev with pointer events',
    function (assert) {
  ...
  QUnit.test(
    'should allow swiperight and call prev with touch events',
    function (assert) {
  ...
  QUnit.test(
    'should allow swipeleft and call next with touch events',
    function (assert) {
      assert.expect(3);
      clearPointerEvents();
      Simulator.setType('touch');
  ...
  QUnit.test('should not call _slide if the carousel is sliding', function (
    assert
  ) {
    assert.expect(1);
  ...
  QUnit.test('should not cycle when there is no attribute data-ride', function (
    assert
  ) {
    assert.expect(1);
  ...
  QUnit.test('should cycle when there is data-ride attribute', function (
    assert
  ) {
    assert.expect(1);
  ...
  QUnit.test('should init carousels with data-ride on load event', function (
    assert
  ) {
    assert.expect(1);
  ...
  QUnit.test(
    'should not add touch event listeners when touch option set to false',
    function (assert) {
      assert.expect(1);

      var spy = sinon.spy(Carousel.prototype, '_addTouchEventListeners');
      var $carousel = $(
        '<div class="carousel" data-ride="carousel" data-touch="false"></div>'
      );
